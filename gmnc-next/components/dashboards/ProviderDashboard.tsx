  ...
  return (
    <div className="space-y-8 pb-10">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white p-6 rounded-2xl border border-slate-100 shadow-sm">
        <div>
          <h1 className="text-2xl font-extrabold text-slate-900 tracking-tight">Clinical Workflow</h1>
        ...
        <div className="flex items-center gap-3">
          <button className="flex items-center gap-2 px-4 py-2 bg-white border border-slate-200 rounded-xl text-[10px] font-bold uppercase tracking-widest text-slate-600 hover:bg-slate-50 transition-colors shadow-sm">
            <PlusIcon className="w-3.5 h-3.5" />
            New Appointment
          </button>
          <button className="px-4 py-2 bg-[#059669] text-white rounded-xl text-[10px] font-bold uppercase tracking-widest shadow-lg shadow-emerald-500/20 hover:opacity-90 transition-opacity flex items-center gap-2">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          <Table 
            ...
            data={APPOINTMENTS}
            columns={[
              { header: 'Time', accessor: 'time', className: 'font-bold text-[#059669]' },
              ...
              { 
                ...
                accessor: (item) => (
                  <span className={cn(
                    ...
                    item.status === 'Confirmed' ? 'bg-amber-50 text-amber-600 border-amber-100' :
            ...
            actions={(item) => (
              <button 
                ...
                className="flex items-center gap-2 px-3 py-1 bg-emerald-50 text-[#059669] border border-emerald-100 rounded-lg text-[9px] font-bold uppercase hover:bg-[#059669] hover:text-white transition-all group"
              >
            ...
            <ChartContainer title="Patient Progress" subtitle="Aggregate neurological index">
              <ResponsiveContainer width="100%" height={250}>
                <LineChart data={REVENUE_DATA}>
                  ...
                  <Line type="monotone" dataKey="subscriptions" stroke="#059669" strokeWidth={3} dot={{ fill: '#059669', strokeWidth: 2, r: 4 }} activeDot={{ r: 6, strokeWidth: 0 }} />
                </LineChart>
              </ResponsiveContainer>
            </ChartContainer>
            ...
            <ChartContainer title="Adherence Rate" subtitle="Exercise completion frequency">
              <ResponsiveContainer width="100%" height={250}>
                <AreaChart data={REVENUE_DATA}>
                  ...
                  <YAxis stroke="#94a3b8" fontSize={11} tickLine={false} axisLine={false} />
                  <Tooltip contentStyle={{ backgroundColor: '#fff', border: '1px solid #e2e8f0', borderRadius: '12px', boxShadow: '0 10px 15px -3px rgb(0 0 0 / 0.1)' }} />
                  <Area type="monotone" dataKey="revenue" stroke="#3b82f6" strokeWidth={2} fillOpacity={1} fill="url(#colorAdh)" />
                </AreaChart>
              </ResponsiveContainer>
            </ChartContainer>
          </div>
        ...
        <div className="space-y-6">
          <div className="bg-white rounded-2xl border border-slate-100 p-6 shadow-sm">
            ...
            <div className="grid grid-cols-1 gap-4">
              {[
                { label: 'Refer Patient', sub: 'Send to specialist', icon: PlusIcon, color: 'rose' },
                { label: 'Clinical Notes', sub: 'Add to recent session', icon: FileTextIcon, color: 'blue' },
                { label: 'Enrollment', sub: 'New patient signup', icon: UserPlusIcon, color: 'emerald' },
              ].map((action, i) => (
                <button key={i} className="flex items-center gap-4 p-4 rounded-xl bg-slate-50 border border-slate-100 hover:border-[#059669]/20 hover:bg-white transition-all group shadow-sm">
                  <div className={cn(
                    "p-2.5 rounded-lg transition-all",
                    action.color === 'rose' ? "bg-emerald-100 text-[#059669] group-hover:bg-[#059669] group-hover:text-white" :
          ...
          <div className="bg-white rounded-2xl border border-slate-100 p-6 overflow-hidden relative group shadow-sm">
