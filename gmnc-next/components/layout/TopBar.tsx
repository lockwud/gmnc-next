...
import { BellIcon, ChevronDownIcon, LayoutDashboardIcon, LogOutIcon, SearchIcon, ShieldCheckIcon, ShieldIcon, UserCheckIcon, UserIcon } from 'lucide-react';
...
export function TopBar() {
  ...
  return (
    <header className="h-[80px] bg-white border-b border-slate-100 flex items-center justify-between px-10 sticky top-0 z-40">
      {/* Centered Search Bar */}
      <div className="absolute left-1/2 -translate-x-1/2 w-full max-w-md hidden md:block">
        <div className="relative group">
          <SearchIcon className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400 group-focus-within:text-[#059669] transition-colors" />
          <input
            type="text"
            placeholder="Search for patient, session or status..."
            className="w-full bg-[#F3F4F6] border-transparent focus:border-[#059669]/20 focus:bg-white focus:ring-4 focus:ring-[#059669]/5 rounded-xl pl-12 pr-4 py-2.5 text-sm transition-all outline-none text-slate-900 font-medium"
          />
        </div>
      </div>
      ...
      <div className="flex items-center gap-6 relative z-10">
        ...
        {user && user.roles.length > 1 && (
          <div className="relative">
            <button
              ...
              className="flex items-center gap-2 px-4 py-2 rounded-xl bg-slate-50 text-slate-700 border border-slate-200 hover:bg-slate-100 transition-all group"
            >
        ...
        <Link 
          href="/notifications"
          className="relative w-10 h-10 flex items-center justify-center text-slate-400 hover:bg-slate-50 hover:text-[#059669] rounded-xl transition-all"
        >
          ...
          <AnimatePresence>
            {isProfileOpen && (
              ...
              >
                ...
                <button className="w-full flex items-center gap-3 px-4 py-2.5 rounded-xl hover:bg-slate-50 text-slate-600 transition-all text-sm font-bold">
                  <UserIcon className="w-4 h-4" />
                  Profile
                </button>
                <button
                  onClick={logout}
                  className="w-full flex items-center gap-3 px-4 py-2.5 rounded-xl hover:bg-emerald-50 text-[#059669] transition-all text-sm font-bold"
                >
